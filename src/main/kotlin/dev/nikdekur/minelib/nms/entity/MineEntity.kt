...
enum class MineEntity(
    ...
    val minecraftKey = MinecraftKey(this.name)
    ...
    companion object {
        ...
        val BY_NMS_CLASS = HashMap<String, MineEntity>().apply {
            MineEntity.entries.forEach {
                val clazz = it.nmsClass ?: return@forEach
                this[clazz.name] = it
            }
        }

        fun from(type: EntityType): MineEntity {
            return MineEntity.valueOf(type.name)
        }

        fun fromNMS(entity: Entity): MineEntity? {
